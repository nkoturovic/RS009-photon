...
int main()
{
    ...
    rs::stdisp.show(img1 << rs::Overlay(rs::Overlay::Color::RED)
                            *rs::Cartoon());
    rs::Display::waitKey();

    rs::stdisp.show(img1 << rs::Neon());
    rs::Display::waitKey();
    
    rs::stdisp.show(img1 << rs::Winter());
    rs::Display::waitKey();

    rs::stdisp.show(img1 << rs::Sepia());
    rs::Display::waitKey();

    rs::stdisp.show(img1 << rs::Silver());
    rs::Display::waitKey();

    rs::stdisp.show(img1 << rs::Rainbow());
    rs::Display::waitKey();

    rs::stdisp.show(img1 << rs::Summer());
    rs::Display::waitKey();
